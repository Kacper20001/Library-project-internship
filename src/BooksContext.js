...
export const BooksProvider = (props) => {
    ...
    const addBookReview = (bookId, review, rating) => {
        const currentBookReviews = bookReviews[bookId] || [];
        currentBookReviews.push({ review, rating });
        setBookReviews({ ...bookReviews, [bookId]: currentBookReviews });
    };
    const updateBookRating = (bookId, newRating) => {
        setBooks(prevBooks => {
            return prevBooks.map(book => {
                if (book.id === bookId) {
                    const updatedReviews = [...book.reviews, { text: "", rating: newRating }];
    ...
    return (
        <BooksContext.Provider value={{books, setBooks, addBook, bookReviews, addBookReview, updateBookRating}}>
            {props.children}
